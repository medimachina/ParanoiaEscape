...
namespace Ferr {
	...
	public class SuperCubeEditor : Editor {
		...
		public override void OnInspectorGUI        () {
			...
			if (mWasStatic != isStatic || serializedObject.ApplyModifiedProperties() || (Event.current.type == EventType.ValidateCommand && Event.current.commandName == "UndoRedoPerformed")) {
				for (int i = 0; i < targets.Length; ++i) {
					SuperCube   box    = (SuperCube)targets[i];
					MeshFilter  filter = box.GetComponent<MeshFilter>();
					RecolorTree tree   = new RecolorTree(filter.sharedMesh, box.transform.localToWorldMatrix);
					box.Build(true, tree);
				}
			}
		...
		private         void OnSceneGUI            () {
			...
			if (box.MeshFilter != null && box.MeshFilter.sharedMesh != null)
				Undo.RecordObject(box.MeshFilter.sharedMesh, "Mesh edit");
			...
			if (Event.current.alt && Event.current.control) {
				...
				TransformHandles(box, rtb, lbf, eventType);
			}
		...
		private void TransformHandles      (SuperCube aBox, Vector3 aRtb, Vector3 aLbf, EventType aEventType) {
			if (Event.current.keyCode == KeyCode.V) return; // don't block vertex snapping with our handles!
			...
			if (aBox.Extents != newExtents) {
				if (mRecolorTree == null) {
					MeshFilter filter = aBox.GetComponent<MeshFilter>();
					mRecolorTree = new RecolorTree(filter.sharedMesh, aBox.transform.localToWorldMatrix);
				}
				...
				aBox.Build(false, mRecolorTree);
				mFullUpdate = true;
			}
			if (aEventType == EventType.MouseUp && mFullUpdate) {
				aBox.Build(true, mRecolorTree);
				mFullUpdate = false;
				mRecolorTree = null;
			}
		}
		private static  bool FaceVisibilityHandles (SuperCube aBox, Vector3 rtb, Vector3 lbf) {
			...
			Handles.color = Color.Lerp(Color.green, Color.white, 0.5f);
			Vector3 pos = aBox.transform.TransformPoint(new Vector3(0, lbf.y, 0));
			Handles.CapFunction cap = (aBox.FaceBottom ? (Handles.CapFunction)SuperCubeEditorUtil.VisibleCap : (Handles.CapFunction)SuperCubeEditorUtil.HiddenCap);
			...
			Handles.color = Color.Lerp(Color.red, Color.white, 0.5f);
			pos = aBox.transform.TransformPoint(new Vector3(rtb.x, 0, 0));
			cap = (aBox.FaceRight ? (Handles.CapFunction)SuperCubeEditorUtil.VisibleCap : (Handles.CapFunction)SuperCubeEditorUtil.HiddenCap);
			if (Handles.Button(pos, Quaternion.identity, HandleUtility.GetHandleSize(pos) * SuperCubeEditorUtil.mHandleSize, HandleUtility.GetHandleSize(pos) * SuperCubeEditorUtil.mHandleSize, cap)) {
			...
			// Left
			pos = aBox.transform.TransformPoint(new Vector3(lbf.x, 0, 0));
			cap = (aBox.FaceLeft ? (Handles.CapFunction)SuperCubeEditorUtil.VisibleCap : (Handles.CapFunction)SuperCubeEditorUtil.HiddenCap);
			if (Handles.Button(pos, Quaternion.identity, HandleUtility.GetHandleSize(pos) * SuperCubeEditorUtil.mHandleSize, HandleUtility.GetHandleSize(pos) * SuperCubeEditorUtil.mHandleSize, cap)) {
			...
			Handles.color = Color.Lerp(Color.blue, Color.white, 0.5f);
			pos = aBox.transform.TransformPoint(new Vector3(0, 0, rtb.z));
			cap = (aBox.FaceBack ? (Handles.CapFunction)SuperCubeEditorUtil.VisibleCap : (Handles.CapFunction)SuperCubeEditorUtil.HiddenCap);
			if (Handles.Button(pos, Quaternion.identity, HandleUtility.GetHandleSize(pos) * SuperCubeEditorUtil.mHandleSize, HandleUtility.GetHandleSize(pos) * SuperCubeEditorUtil.mHandleSize, cap)) {
			...
			// Front
			pos = aBox.transform.TransformPoint(new Vector3(0, 0, lbf.z));
			cap = (aBox.FaceFront ? (Handles.CapFunction)SuperCubeEditorUtil.VisibleCap : (Handles.CapFunction)SuperCubeEditorUtil.HiddenCap);
			if (Handles.Button(pos, Quaternion.identity, HandleUtility.GetHandleSize(pos) * SuperCubeEditorUtil.mHandleSize, HandleUtility.GetHandleSize(pos) * SuperCubeEditorUtil.mHandleSize, cap)) {
			...
			Handles.color = Color.Lerp(Color.green, Color.white, 0.5f);
			pos = aBox.transform.TransformPoint(new Vector3(0, rtb.y, 0));
			cap = (aBox.FaceTop ? (Handles.CapFunction)SuperCubeEditorUtil.VisibleCap : (Handles.CapFunction)SuperCubeEditorUtil.HiddenCap);
			if (Handles.Button(pos, Quaternion.identity, HandleUtility.GetHandleSize(pos) * SuperCubeEditorUtil.mHandleSize, HandleUtility.GetHandleSize(pos) * SuperCubeEditorUtil.mHandleSize, cap)) {
		...
		private void CheckScale(SuperCube aCube) {
			if (aCube != null && aCube.transform.localScale != Vector3.one) {
				...
				MeshFilter  filter = aCube.GetComponent<MeshFilter>();
				RecolorTree tree   = new RecolorTree(filter.sharedMesh, aCube.transform.localToWorldMatrix);
				aCube.Build(true, tree);
			}
		}
			...
			if (Event.current.type == EventType.DragUpdated || Event.current.type == EventType.DragPerform) {// || Event.current.type == EventType.Repaint) {
				...
				if (dragMat != null) {
					...
					if (mRecolorTree == null) {
						...
						mRecolorTree = new RecolorTree(filter.sharedMesh, cube.transform.localToWorldMatrix);
					}
					cube.Build(false, mRecolorTree);
				}
			}
			if (Event.current.type == EventType.DragExited || Event.current.type == EventType.DragPerform) {
				cube.Build(true, mRecolorTree);
				mRecolorTree = null;
			}
		}
		...
		private static bool CheckSide(Ray aRay, Vector3 dir, float aDist, float aRadius, float closest, out float dist) {
			...
			if (p.Raycast(aRay, out dist)) {
				if (dist <= closest && (center - aRay.GetPoint(dist)).magnitude < aRadius) {
					Handles.SphereHandleCap(0,aRay.GetPoint(dist), Quaternion.identity, 0.2f, Event.current.type);
					return true;
				}
			}
