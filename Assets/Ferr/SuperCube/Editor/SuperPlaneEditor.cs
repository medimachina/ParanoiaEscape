...
namespace Ferr {
	...
	public class SuperPlaneEditor : Editor {
		...
		bool        mNeedsFullUpdate = false;
		RecolorTree mRecolorTree     = null;
		#endregion
		...
		public override void OnInspectorGUI        () {
			...
			if (mWasStatic != isStatic || serializedObject.ApplyModifiedProperties() || GUI.changed || (Event.current.type == EventType.ValidateCommand && Event.current.commandName == "UndoRedoPerformed")) {
				for (int i = 0; i < targets.Length; ++i) {
					SuperPlane  p      = (SuperPlane)targets[i];
					MeshFilter  filter = p.GetComponent<MeshFilter>();
					RecolorTree tree   = new RecolorTree(filter.sharedMesh, p.transform.localToWorldMatrix);
					p.Build(true, tree);
				}
			}
		...
		private void TransformHandles      (SuperPlane aPlane, Vector3 rtb, Vector3 lbf, EventType aEventType) {
			...
			if (aPlane.Extents != newExtents) {
				if (mRecolorTree == null) {
					MeshFilter filter = aPlane.GetComponent<MeshFilter>();
					mRecolorTree = new RecolorTree(filter.sharedMesh, aPlane.transform.localToWorldMatrix);
				}
				...
				aPlane.Build(false, mRecolorTree);
				mNeedsFullUpdate = true; 
			}
			
			if (aEventType == EventType.MouseUp && mNeedsFullUpdate) {
				aPlane.Build(true, mRecolorTree);
