...
namespace Ferr {
	static class SuperCubeEditorUtil {
		...
		#region Cap methods
		internal static void Handle(PivotType aSides, Transform aTransform, bool aLockX, bool aLockY, bool aLockZ, Vector3 aSnap, ref Vector3 lbf, ref Vector3 rtb, Handles.CapFunction aCap) {
		...
		internal static void HiddenCap (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, EventType aCurrent) {
			EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, mTexHidden, aCurrent);
		}
		internal static void VisibleCap(int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, EventType aCurrent) {
			EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, mTexVisible, aCurrent);
		}
		internal static void AddCap    (int aControlID, Vector3 aPosition, Quaternion aRotation, float aSize, EventType aCurrent) {
			EditorTools.ImageCapBase(aControlID, aPosition, aRotation, aSize, mTexAdd, aCurrent);
		}
		#endregion
