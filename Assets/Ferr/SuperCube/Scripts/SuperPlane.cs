...
public class SuperPlane : MonoBehaviour, IProceduralMesh {
	...
	public void ForceNewMesh() {
		...
		ProceduralMeshUtil.MakeInstName(filter);
	...
	public void Build(bool aFullBuild) {
		Build(aFullBuild, null);
	}
	/// <summary>
	/// Creates the SuperPlane mesh based on the existing parameters. aFullBuild tells it whether or not to calculate some of the more time expensive items, such as lightmap UVs.
	/// </summary>
	/// <param name="aFullBuild">Do time consuming calculations (lightmap UVs), or just the bare essentials?</param>
	/// <param name="aRecolorTree">A K-d tree with color data for assigning colors to the verts. Can be null.</param>
	public void Build       (bool aFullBuild, RecolorTree aRecolorTree) {
		MeshFilter filter = GetComponent<MeshFilter>();
		ProceduralMeshUtil.EnsureProceduralMesh(filter, false);
		...
		float uvRatio = 1;
		if (r.sharedMaterial != null && r.sharedMaterial.mainTexture != null && r.sharedMaterial.mainTexture.height > 0)
		...
		if (aRecolorTree != null) {
			aRecolorTree.Recolor(verts, ref colors, transform.localToWorldMatrix);
		} else {
		...
		#if UNITY_EDITOR
		if (verts.Count > 500 && aFullBuild && UnityEditor.GameObjectUtility.AreStaticEditorFlagsSet( gameObject, UnityEditor.StaticEditorFlags.ContributeGI )) {
		...
		m.RecalculateBounds();
		#if UNITY_EDITOR
		if (aFullBuild && UnityEditor.GameObjectUtility.AreStaticEditorFlagsSet( gameObject, UnityEditor.StaticEditorFlags.ContributeGI )) {
